...
// Support cross-origin loading: NAS_BASE is set by the inject blob shim
// When loaded same-origin, importScripts uses relative path (works normally)
if (typeof NAS_BASE !== 'undefined') {
  importScripts(NAS_BASE + 'randomx_fast.js');
} else {
...
// Inject script can override base URL for cross-origin worker loading
...
async function initEngine(seedHashHex) {
  post('status', { message: 'Loading JIT RandomX engine...' });
    const baseUrl = baseUrlOverride || self.location.href.replace(/worker\.js.*$/, '').replace(/\?.*$/, '').replace(/#.*$/, '');
  M = await createRandomXFast((f) => baseUrl + f);
  // When running as a blob worker (cross-origin inject), self.location is a blob: URL.
  // NAS_BASE is prepended by the blob shim so importScripts + fetch resolve to NAS.
...
self.onmessage = async (e) => {
  ...
  try {
    if (msg.type === 'init') {
      ...
      // Update nonce range if server assigned one (pooled mining)
      if (msg.job.nonce_start !== undefined && msg.job.nonce_end !== undefined) {
        nonceStart = msg.job.nonce_start;
        nonceEnd = msg.job.nonce_end;
        nonceCur = nonceStart;
      }
      ...
      mining = false;
    } else if (msg.type === 'baseUrl') {
      baseUrlOverride = msg.url;
